import { useScrollAnimation, useHoverAnimation } from '@/hooks/useScrollAnimation';
...
const ANIMATION_PRESETS = {
  ...
  hero: {
    ...
    subtitle: { animationType: 'slideUp' as const, duration: 600, threshold: 0.1 },
  ...
  cta: {
    ...
    container: { animationType: 'slideUp' as const, duration: 600, threshold: 0.2 },
  ...
  section: {
    ...
    content: { animationType: 'slideUp' as const, duration: 600, threshold: 0.2 },
  },
  // General animations
  general: {
...
export function useHomeAnimations() {
  ...
  const ctaContainerAnimation = useScrollAnimation(ANIMATION_PRESETS.cta.container);
  const ctaStatsAnimation = useScrollAnimation(ANIMATION_PRESETS.cta.stats);

  const sectionHeaderAnimation = useScrollAnimation(ANIMATION_PRESETS.section.header);
  const sectionTabsAnimation = useScrollAnimation(ANIMATION_PRESETS.section.tabs);
  const sectionContentAnimation = useScrollAnimation(ANIMATION_PRESETS.section.content);
  ...
  const secondaryButtonHover = useHoverAnimation({ scale: 1.02, duration: 200 });
  const cardHover = useHoverAnimation({ scale: 1.02, duration: 300 });

  // Utility functions for custom animations (these don't call hooks)
  const getHeroAnimationConfig = (type: keyof typeof ANIMATION_PRESETS.hero, config?: Partial<AnimationConfig>) => {
    return { ...ANIMATION_PRESETS.hero[type], ...config };
  };

  const getCTAAnimationConfig = (type: keyof typeof ANIMATION_PRESETS.cta, config?: Partial<AnimationConfig>) => {
    return { ...ANIMATION_PRESETS.cta[type], ...config };
  };

  const getSectionAnimationConfig = (type: keyof typeof ANIMATION_PRESETS.section, config?: Partial<AnimationConfig>) => {
    return { ...ANIMATION_PRESETS.section[type], ...config };
  };

  const getGeneralAnimationConfig = (type: keyof typeof ANIMATION_PRESETS.general, config?: Partial<AnimationConfig>) => {
