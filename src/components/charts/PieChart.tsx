...
const PieChart = ({ data, size = 200, showLabels = true, className = '' }: IPieChartProps) => {
    ...
    <div className={`flex flex-col items-center ${className}`}>
      ...
      {showLabels && (
        <div className="mt-4 space-y-2 w-full">
          {chartData.map((item, index) => (
            <div key={`label-${item.label}-${index}`} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                <div 
                  className="w-3 h-3 rounded-sm" 
                  style={{ backgroundColor: item.color }}
                />
                <span style={{ color: getChartTextColor('muted') }}>{item.label}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-medium">{item.value}</span>
                <span className="text-xs" style={{ color: getChartTextColor('muted') }}>
