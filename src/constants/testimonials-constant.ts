...
export const validateTestimonialData = (
  ...
  data.forEach((testimonial: unknown, index: number) => {
    ...
    if (!validation.isValid) {
      errors.push(`Testimonial ${index + 1}: ${validation.errors.join(", ")}`);
    }
  });
...
export const safeGetTestimonials = (data: Testimonial[]): Testimonial[] => {
  try {
    ...
    toast.error("Failed to access testimonial data");
    return [];
  }
};
...
if (!validation.isValid) {
  toast.error("Testimonial data validation failed: " + validation.errors);
}
